...
class SSDPredictShow():
    ...
    def show(self, img, width, height, channel, data_confidence_level):
        """
        物体検出の予測結果を表示をする関数。
        ...
        rgb_img, predict_bbox, pre_dict_label_index, scores = self.ssd_predict(
            img, width, height, channel, data_confidence_level)
#         print(predict_bbox)
        ...
        return fig
    def ssd_predict(self, img, width, height, channel, data_confidence_level=0.5):
        """
        SSDで予測させる関数。
