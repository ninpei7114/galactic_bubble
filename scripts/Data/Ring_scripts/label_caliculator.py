...
def calc_pix(row, world, GLON_min,GLON_max,GLAT_min,GLAT_max, rank, mode):
    """
    切り出す画像の範囲をここで決める
    ...
    if rank==3:
        while ok:
            if mode=='train':
#                 random_num = 1/0.9 
                random_num = 1/np.random.uniform(0.5, 0.89)
            else:
#                 random_num = 1/np.random.uniform(0.8, 0.95)
                random_num = 1/np.random.uniform(0.5, 0.89)
        ...
        while ok:
            if mode=='train':
                ...
                random_num = 1/np.random.uniform(0.3, 0.89)
            lmax = row['GLON'] + random_num*1.5*row['Rout']/60
            bmin = row['GLAT'] - random_num*1.5*row['Rout']/60
            #右端
            lmin = row['GLON'] - random_num*1.5*row['Rout']/60
