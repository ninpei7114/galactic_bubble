...
class ResNet(nn.Module):
    def __init__(self, backbone='resnet18', backbone_path=None):
        ...
        self.feature_extractor[0] = nn.Conv2d(2, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
...
class SSD(nn.Module):
    ...
    def __init__(self, phase, cfg, backbone=ResNet('resnet18')):
        super().__init__()
        
        self.phase = phase
        self.feature_extractor = backbone
        self.label_num = cfg["num_classes"]
       
        self._build_additional_features(self.feature_extractor.out_channels)
        self.num_defaults = cfg['bbox_aspect_num']
        self.loc = []
        self.conf = []
...
#         print('confs', confs.shape)
        ...
        output = (locs, confs, self.dbox_list)
        ...
        if self.phase == "inference":  # 推論時
            ...
            return output, torch.cat([decode(locs[rr].to('cpu'), self.dbox_list)[None] for rr in range(locs.shape[0])], axis=0)
...
#         return loss_c_avg, loss_l_avg, loss_c_posi.mean(), loss_c_nega.mean()
...
#         print('pos_idx', pos_idx)
        ...
        batch_conf = conf_data.contiguous().view(-1, num_classes)
